...
For Type 0: 
- If the question requires subtitle-related information, first call the SubtitleAgent to obtain the complete subtitles (Set start_time to 0). Then !!compare the subtitles in the question/options with the subtitles from SubtitleAgent!! to obtain the key timestamps.
...
- If the question requires a global and rough understanding of the video, call PerceptionAgent to perceive the entire video globally, making sure to specify in the `instruct` field that global video perception is required.
...
2. In cases where different segments lead to different options (contradictory information), use PerceptionAgent and specify in the instruct that you need to sequentially identify and resolve conflicts. Provide all time segments that need to be perceived in the instruct, then synthesize the results to produce a unique answer.
...
7. Only proceed with an answer once you have **sufficient evidence**. For example, if asked "Is the first one xxx?", do not simply confirm the presence of "xxx"—instead, determine which occurrence is truly the first before responding.
